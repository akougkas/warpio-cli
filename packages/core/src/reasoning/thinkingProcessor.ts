...
import { WarpioReasoningRegistry, ReasoningCapability } from './modelCapabilities.js';
...
export class WarpioThinkingProcessor {
  ...
  async *processStream(
    ...
    const timeoutPromise = new Promise<never>((_, reject) => {
      setTimeout(() => {
        reject(new Error(`Thinking processor timeout after ${this.options.timeoutMs}ms`));
    ...
    try {
      ...
      if (error instanceof Error && error.message.includes('timeout')) {
        console.warn('Thinking processor timed out, continuing with normal stream processing');
      }
    }
  }
      ...
      for (const token of tokens) {
        ...
        if (token.type === 'thinking') {
          ...
          if (this.thinkingTokenCount > this.options.maxThinkingTokens!) {
            if (this.options.enableDebug) {
              console.warn(`Thinking tokens exceeded limit (${this.thinkingTokenCount} > ${this.options.maxThinkingTokens}), truncating`);
            }
  ...
  private extractTokens(): ThinkingToken[] {
    ...
    for (const pattern of this.capability.thinkingPatterns) {
      ...
      if (match) {
        ...
        if (this.options.enableDebug) {
          console.log(`Extracted thinking token: ${subject} (${tokenCount} tokens)`);
        }
  ...
  private mightContainThinking(): boolean {
    ...
    return patterns.some(p => this.buffer.toLowerCase().includes(p.toLowerCase()));
  }
