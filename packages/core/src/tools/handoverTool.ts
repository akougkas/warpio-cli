...
const handoverToPersonaSchema = z.object({
  targetPersona: z.string().describe('Target persona to hand over to (e.g., data-expert, analysis-expert)'),
  taskDescription: z.string().describe('Specific task for the target persona to execute'),
  artifacts: z.array(z.object({
    path: z.string(),
    type: z.enum(['input', 'output', 'intermediate']),
    format: z.string(),
    metadata: z.record(z.unknown()).optional()
  })).optional().describe('Files and data to pass to target persona'),
  environment: z.object({
    variables: z.record(z.string()).optional(),
    workingDirectory: z.string().optional(),
    timeout: z.number().optional()
  }).optional().describe('Execution environment for target persona'),
  mode: z.enum(['synchronous', 'asynchronous']).default('synchronous').describe('Execution mode'),
  interactive: z.boolean().default(false).describe('Whether target persona should run interactively'),
  scientificContext: z.object({
    dataFormats: z.array(z.string()).optional(),
    hpcEnvironment: z.object({
  ...
  }).optional().describe('Scientific computing context for HPC/data workflows')
});

type HandoverToPersonaParams = z.infer<typeof handoverToPersonaSchema>;
...
export class HandoverToPersonaTool extends BaseTool<HandoverToPersonaParams, HandoverToolResult> {
    ...
    const schema: FunctionDeclaration = {
      ...
      parameters: {
        ...
        properties: {
          targetPersona: {
            type: Type.STRING,
            description: 'Target persona to hand over to (e.g., data-expert, analysis-expert)'
          },
          taskDescription: {
            type: Type.STRING,
  ...
  async validate(params: HandoverToPersonaParams): Promise<void> {
    ...
    if (timeout && (timeout < 1000 || timeout > 3600000)) { // 1s to 1h
  ...
  async execute(
    ...
    updateCallback?.(`🚀 Initiating handover to persona: ${params.targetPersona}`);
    ...
    try {
      ...
      updateCallback?.(`📦 Preparing context from ${currentPersonaName} persona`);
      ...
      const context: PersonaContext = {
        metadata: {
          ...
          workingDirectory: params.environment?.workingDirectory || process.cwd(),
        ...
        scientific: {
          ...
          hpcEnvironment: params.scientificContext?.hpcEnvironment ? {
            ...
            walltime: params.scientificContext.hpcEnvironment.walltime || '01:00:00',
            queue: params.scientificContext.hpcEnvironment.queue || 'default'
          } : undefined,
      ...
      updateCallback?.(`⚡ Using MessagePack serialization for 3-5x performance gain`);

      // Execute handover with performance monitoring
      const startTime = Date.now();
      const result = await contextHandoverService.handoverToPersona(context, {
      ...
      return {
        ...
        llmContent: [{ text: result.output || `Task completed with status: ${result.status}` }],
   ...
   */
  private async buildFileReferences(artifacts: HandoverToPersonaParams['artifacts'] = []) {
    const fs = await import('fs').then(m => m.promises);
  ...
  async shouldConfirmExecute(
    ...
    if (hasFileOutputs || isLongRunning || !!isHPCJob) {
      return {
        ...
        prompt: `This will hand over context to ${params.targetPersona} persona with the following characteristics:\n\n` +
